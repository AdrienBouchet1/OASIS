...
class EM_Dataset(torch.utils.data.Dataset): 
        ...
        def __init__(self, opt, for_metrics):
                ...
                assert opt.output_channel in [1,3],"problème de channel"
                self.opt = opt
        ...
        def list_images(self):
            ...
            path_img = os.path.join(self.opt.dataroot, "images")
            path_lab = os.path.join(self.opt.dataroot, "labels")

            img_list = os.listdir(path_img)
            lab_list = os.listdir(path_lab)

            img_list = [filename for filename in img_list if ".tif" in filename ]
            lab_list = [filename for filename in lab_list if ".tif" in filename] ## changer l'exntension si problème
        ...
        def transforms(self, image, label):
            ...
            # normalize
            if image.shape[0] == 1:
                 image = TR.functional.normalize(image, (0.5,), (0.5,)) ### Si jamais on n'a qu'un channel
            else:
